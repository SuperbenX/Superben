...
const App: React.FC = () => {
  ...
  const initAppData = useCallback(async () => {
    ...
    if (todayMoment && todayMoment.dateISO === todayStr) return;
  ...
  const renderActiveTab = () => {
    switch (activeTab) {
      case 'home': return <Home dailyData={todayMoment} onPlay={handlePlay} onOpenHarbor={() => setIsHarborVisible(true)} onEnterRipple={() => setIsRipplePlayerVisible(true)} />;
      case 'library': return <Library onPlay={(item, skip, presc) => handlePlay(item, !!skip, false, presc || "")} onNavigate={setActiveTab} />;
      case 'deep_dive': return <DeepDive onPlay={handlePlay} />;
      case 'qa': return <InteractionQA messages={qaMessages} setMessages={setQaMessages} prescription={qaPrescription} setPrescription={setQaPrescription} onPlay={(item, skip, presc) => handlePlay(item, skip, false, presc)} />;
      case 'history': return <History />;
      case 'settings': return <Settings onNavigate={setActiveTab} />;
      case 'admin': return <Admin />;
      default: return <Home dailyData={todayMoment} onPlay={handlePlay} onOpenHarbor={() => setIsHarborVisible(true)} onEnterRipple={() => setIsRipplePlayerVisible(true)} />;
    }
  };
          ...
          {isSyncing && activeTab === 'home' && !todayMoment && (
            <div className="fixed top-8 left-1/2 -translate-x-1/2 z-[150] animate-in fade-in duration-500">
              <div className="px-4 py-1.5 bg-white/5 backdrop-blur-xl border border-white/5 rounded-full flex items-center gap-3">
                 ...
                 <span className="text-[8px] text-white/30 tracking-[0.3em] font-bold uppercase">对齐今日星历</span>
              </div>
            </div>
          )}
